...
싱글톤 빈과 프로토타입 빈을 함께 사용할 때, 어떻게 하면 사용할 떄마다 항상 새로운 프로토타입 빈을 생성할 수 있을까?
...
지정한 빈을 컨테이너에서 대신 찾아주는 DL 서비스를 제공하는 것이 바로 `ObjectProvider` 다. 참고로 과거에는 `ObjectFactory` 가 있었는데, 여기에 편의 기능을 추가해서 `ObjectProvider`가 만들어졌다.
...
public int logic() {
  PrototypeBena prototypeBean = prototypeBeanProvider.getObject();
  prototypeBean.addCount();
  int count = prototypeBean.getCount();
  return count;
}
...
- javax.inject.Provder 참고용 코드

  ``` java
  package javax.inject;
  public interface Provder<T> {
    T get();
  }
  ```
...
- `provder`의 `get()` 을 호출하면 내부에서는 스프링 컨테이너를 통해 해당 빈을 찾아서 반환한다. (DL)
...
public class MyLogger {
    ...
    public void init() {
        ...
        System.out.println("[" + uuid + "] request scope bean careate:" + this);
    }
...
- 비즈니스 로직이 있는 서비스 계층에서도 로그를 출려갷보자.
...
스프링 애플리케이션을 실행시키면 오류가 발생한다. 스프링 애플리케이션을 실행하는 시점에 싱글톤 빈은 생성해서 주입이 가능하지만, request 스코프 빈은 아직 생성되지 않는다. 이 빈은 실제 고객의 요청이 와야 생성할 수 있다!
...
- 그리고 나머지 코드들을 Provider 사용 이전으로 롤백한다.
...
- myLogger.getClass()로 콘솔에 출력해보면 우리가 등록한 순수한 MyLogger 클래스가 아니라 `MyLogger$EnhancerBySpringCGLIB` 라는 클래스로 만들어진 객체가 대신 등록된 것을 확인할 수 있다.
- 그리고 스프링 컨테이너에 "myLogger" 라는 일므으로 진짜 대신에 이 가짜 프록시 객체를 등록한다.
...
- CGLIB 라는 라이브러리로 내 클래스를 상속 받은 가짜 프록시 객체를 만들어서 중비한다.
...
- 사실 provider를 사용하든, 프록시를 사욯아든 핵심 아이디어는 진짜 객체 조회를 꼭 필요한 시점까지 지연처리 한다는 점이다.
